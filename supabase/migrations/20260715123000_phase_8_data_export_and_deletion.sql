...
alter table public.altr_deletion_requests
  ...
  add constraint altr_deletion_requests_requested_scope_check
    check (requested_scope in ('all','account','conversations','memory')),
  drop constraint if exists altr_deletion_requests_source_check,
  add constraint altr_deletion_requests_source_check
    check (source in ('authenticated','public')),
  drop constraint if exists altr_deletion_requests_verification_state_check,
  add constraint altr_deletion_requests_verification_state_check
    check (verification_state in ('pending','verified','not_required','failed'));

create unique index if not exists altr_deletion_requests_public_reference_key
  on public.altr_deletion_requests(public_reference)
  where public_reference is not null;
create index if not exists altr_deletion_requests_email_created_idx
  on public.altr_deletion_requests(lower(email), created_at desc)
...
create index if not exists altr_deletion_request_history_request_idx
  on public.altr_deletion_request_history(request_id, created_at);

drop policy if exists "deletion request history own select" on public.altr_deletion_request_history;
create policy "deletion request history own select"
  on public.altr_deletion_request_history for select to authenticated
  using (exists (
    select 1 from public.altr_deletion_requests r
...
create policy "deletion requests own select"
  on public.altr_deletion_requests for select to authenticated
...
alter table public.altr_subscriptions add column if not exists deleted_subject_hash text;
alter table public.altr_invoices add column if not exists deleted_subject_hash text;
alter table public.altr_billing_orders add column if not exists deleted_subject_hash text;
alter table public.altr_billing_invoices add column if not exists deleted_subject_hash text;
