...
import { BrainCircuit, CalendarDays, Check, CreditCard, Database, Home, LockKeyhole, LogOut, Mail, MessageSquareText, Save, Settings2, ShieldCheck, Sparkles, Trash2, UsersRound } from "lucide-react";
...
import { AltrProfile, getCurrentProfile, hasPlanAccess, PlanId, signOutAccount, ToneMode, updateCurrentProfile } from "@/lib/auth";
import { hasValidConsent } from "@/lib/legal/consent-store";

type Tab = "overview" | "data" | "privacy" | "settings";
const menu = [
  { id: "overview" as Tab, label: "Мій Altr", icon: BrainCircuit },
  { id: "data" as Tab, label: "Дані й памʼять", icon: Database },
  { id: "privacy" as Tab, label: "Приватність і дані", icon: ShieldCheck },
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const conversationAllowed = hasValidConsent(current.id, "conversation_processing");
    const memoryAllowed = hasValidConsent(current.id, "ai_memory");
    if (!conversationAllowed || !memoryAllowed) {
      current = updateCurrentProfile({
        connections: conversationAllowed ? current.connections : { email: false, calendar: false, messages: false, workspace: false },
        preferences: memoryAllowed ? current.preferences : { ...current.preferences, learning: false, autoDrafts: false },
      }) ?? current;
    }
    setProfile(current);
  }, [router]);
  ...
  const requireConsent = (type: "conversation_processing" | "ai_memory", action: () => void) => {
    if (!hasValidConsent(profile.id, type)) {
      setPrivacyNotice(type === "conversation_processing" ? "Спочатку надай окрему згоду на обробку переписок." : "Спочатку надай окрему згоду на створення персональної AI-памʼяті.");
      setTab("privacy");
      return;
    }
    setPrivacyNotice("");
      ...
      <div className="relative z-10 lg:pl-[272px]">
        ...
        <div className="mx-auto max-w-[1400px] px-4 py-8 sm:px-7 lg:px-10">
          ...
          {tab === "data" && <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
            ...
            ].map(([key,Icon,name,description,requiredPlan,feature]) => { const k = key as keyof AltrProfile["connections"]; const I = Icon as typeof Mail; const required = requiredPlan as PlanId; const locked = !hasPlanAccess(profile.plan, required); return <Panel key={k} className={`relative p-6 ${locked ? "locked-feature" : ""}`}><div className="flex justify-between"><span className="flex h-11 w-11 items-center justify-center rounded-xl border border-cyan-100/[.09] bg-cyan-200/[.035]"><I className="h-5 w-5 text-cyan-100/55" /></span>{locked ? <button onClick={() => router.push(`/pricing?feature=${String(feature)}`)} className="locked-badge"><LockKeyhole className="h-3 w-3" />{planNames[required]}</button> : <Toggle label={String(name)} active={profile.connections[k]} onClick={() => gated(required, String(feature), () => requireConsent("conversation_processing", () => requireConsent("ai_memory", () => persist({ connections: { ...profile.connections, [k]: !profile.connections[k] } }))))} />}</div><h2 className="mt-7 text-xl">{String(name)}</h2><p className="mt-2 min-h-12 text-sm leading-6 text-white/35">{String(description)}</p><button onClick={() => gated(required, String(feature), () => requireConsent("conversation_processing", () => requireConsent("ai_memory", () => persist({ connections: { ...profile.connections, [k]: !profile.connections[k] } }))))} className={`mt-6 text-xs ${locked ? "text-cyan-100/48" : profile.connections[k] ? "text-cyan-100/55" : "text-white/25"}`}>{locked ? "Відкрити з вищим планом →" : profile.connections[k] ? "● Підключено" : "○ Не підключено"}</button></Panel>})}</div>
          ...
          {tab === "privacy" && <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
            {privacyNotice && <div role="status" className="mb-5 rounded-xl border border-cyan-100/[.09] bg-cyan-200/[.035] px-4 py-3 text-sm text-cyan-50/65">{privacyNotice}</div>}
            <PrivacySettingsPanel profile={profile} onProfileChange={setProfile} />
          </motion.div>}
          ...
          {tab === "settings" && <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <div className="mt-8 grid gap-4 xl:grid-cols-[1.1fr_.9fr]">
              ...
              <div className="space-y-4"><Panel className="p-6 sm:p-8"><span className="data-label">ALTR BEHAVIOR</span><h2 className="mt-3 text-2xl">Поведінка системи</h2><div className="mt-7 space-y-6">{[["learning","Безперервне навчання","free","learning"],["autoDrafts","Автоматичні чернетки","personal","drafts"],["weeklyDigest","Щотижневий підсумок","personal","digest"],["privacyMode","Приватний режим","free","privacy"]].map(([key,label,requiredPlan,feature]) => { const k=key as keyof AltrProfile["preferences"]; const required=requiredPlan as PlanId; const locked=!hasPlanAccess(profile.plan,required);return <div key={key} className="flex items-center justify-between gap-4"><span className="flex items-center gap-2 text-sm">{label}{locked && <LockKeyhole className="h-3.5 w-3.5 text-cyan-100/40" />}</span>{locked ? <button onClick={() => router.push(`/pricing?feature=${feature}`)} className="locked-badge">{planNames[required]}</button> : <Toggle label={label} active={profile.preferences[k]} onClick={() => gated(required,feature,() => k === "learning" ? requireConsent("ai_memory", () => persist({preferences:{...profile.preferences,[k]:!profile.preferences[k]}})) : k === "autoDrafts" ? requireConsent("conversation_processing", () => requireConsent("ai_memory", () => persist({preferences:{...profile.preferences,[k]:!profile.preferences[k]}}))) : persist({preferences:{...profile.preferences,[k]:!profile.preferences[k]}}))} />}</div>})}</div></Panel><Panel className="p-6"><div className="mb-5 flex items-center justify-between"><div><span className="data-label">CURRENT PLAN</span><p className="mt-2 text-lg">{planNames[profile.plan]}</p></div><Link href="/pricing" className="future-button rounded-full px-4 py-2 text-xs">Змінити план</Link></div><div className="flex flex-wrap gap-3 border-t border-white/[.06] pt-5"><button onClick={leave} className="glass-button inline-flex items-center gap-2 rounded-full px-5 py-3 text-sm"><LogOut className="h-4 w-4" />Вийти</button><Link href="/delete-data" className="danger-button inline-flex items-center gap-2 rounded-full px-5 py-3 text-sm"><Trash2 className="h-4 w-4" />Видалити акаунт і дані</Link></div></Panel></div>
            </div>
          </motion.div>}
        </div>
      </div>
