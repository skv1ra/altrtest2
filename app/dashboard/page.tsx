...
import { BrainCircuit, CalendarDays, Check, Database, Home, LogOut, Mail, MessageSquareText, Save, Settings2, ShieldCheck, Sparkles, Trash2 } from "lucide-react";
import Link from "next/link";
import { useRouter } from "next/navigation";
import { ReactNode, useEffect, useState } from "react";
import { AiMark } from "@/components/Navigation";
import { AltrProfile, deleteCurrentAccount, getCurrentProfile, signOutAccount, ToneMode, updateCurrentProfile } from "@/lib/auth";
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#05080c] text-white">
      ...
      <aside className="dashboard-sidebar fixed inset-y-0 left-0 z-30 hidden w-[272px] border-r border-cyan-100/[.065] p-5 lg:flex lg:flex-col">
        ...
        <nav className="mt-10 space-y-2">{menu.map(({ id, label, icon: Icon }) => <button key={id} onClick={() => setTab(id)} className={`dashboard-nav-item w-full ${tab === id ? "dashboard-nav-active" : ""}`}><Icon className="h-[18px] w-[18px]" />{label}{tab === id && <span className="status-dot-css ml-auto" />}</button>)}</nav>
        <div className="mt-auto"><div className="mb-4 rounded-[1.35rem] border border-cyan-100/[.07] bg-white/[.025] p-4"><span className="data-label">SYSTEM</span><p className="mt-4 text-sm">Навчання активне</p><p className="mt-1 text-xs leading-5 text-white/30">Altr аналізує лише дозволені джерела.</p></div><button onClick={leave} className="dashboard-nav-item w-full"><LogOut className="h-4 w-4" />Вийти</button></div>
      </aside>
      ...
      <div className="relative z-10 lg:pl-[272px]">
        <header className="sticky top-0 z-20 border-b border-cyan-100/[.055] bg-[#05080c]/80 px-4 backdrop-blur-2xl sm:px-7 lg:px-10">
          <div className="mx-auto flex h-[82px] max-w-[1400px] items-center justify-between"><Link href="/" className="flex items-center gap-2 font-semibold lg:hidden">Altr <AiMark /></Link><p className="hidden text-xs uppercase tracking-[.18em] text-white/27 lg:block">Особистий кабінет / <span className="text-cyan-100/52">{menu.find(x => x.id === tab)?.label}</span></p><div className="flex items-center gap-3"><Link href="/" className="profile-action hidden sm:flex" aria-label="На головну"><Home className="h-4 w-4" /></Link><button onClick={() => setTab("settings")} className="flex items-center gap-3 rounded-full border border-white/[.07] bg-white/[.028] py-1.5 pl-1.5 pr-4"><span className="flex h-9 w-9 items-center justify-center rounded-full border border-cyan-100/15 bg-cyan-200/[.07]">{profile.name[0]?.toUpperCase()}</span><span className="hidden text-left sm:block"><b className="block text-sm font-medium">{profile.name}</b><small className="text-[10px] uppercase tracking-[.14em] text-white/28">Altr ID active</small></span></button></div></div>
          <nav className="flex gap-1 overflow-x-auto pb-3 lg:hidden">{menu.map(({ id, label, icon: Icon }) => <button key={id} onClick={() => setTab(id)} className={`mobile-dashboard-tab ${tab === id ? "mobile-dashboard-tab-active" : ""}`}><Icon className="h-4 w-4" />{label}</button>)}</nav>
        </header>
        ...
        <div className="mx-auto max-w-[1400px] px-4 py-8 sm:px-7 lg:px-10">
          ...
          {tab === "data" && <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <div className="mt-8 grid gap-4 lg:grid-cols-3">{[
              ["email",Mail,"Email","Тон, листування й робочі домовленості."],
              ["calendar",CalendarDays,"Календар","Зустрічі, ритм дня й важливі контексти."],
              ["messages",MessageSquareText,"Повідомлення","Живий стиль спілкування з людьми."],
            ].map(([key,Icon,name,description]) => { const k = key as keyof AltrProfile["connections"]; const I = Icon as typeof Mail; return <Panel key={k} className="p-6"><div className="flex justify-between"><span className="flex h-11 w-11 items-center justify-center rounded-xl border border-cyan-100/[.09] bg-cyan-200/[.035]"><I className="h-5 w-5 text-cyan-100/55" /></span><Toggle label={String(name)} active={profile.connections[k]} onClick={() => persist({ connections: { ...profile.connections, [k]: !profile.connections[k] } })} /></div><h2 className="mt-7 text-xl">{String(name)}</h2><p className="mt-2 min-h-12 text-sm leading-6 text-white/35">{String(description)}</p><p className={`mt-6 text-xs ${profile.connections[k] ? "text-cyan-100/55" : "text-white/25"}`}>{profile.connections[k] ? "● Підключено" : "○ Не підключено"}</p></Panel>})}</div>
          ...
          {tab === "settings" && <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <div className="mt-8 grid gap-4 xl:grid-cols-[1.1fr_.9fr]">
              ...
              <div className="space-y-4"><Panel className="p-6 sm:p-8"><span className="data-label">ALTR BEHAVIOR</span><h2 className="mt-3 text-2xl">Поведінка системи</h2><div className="mt-7 space-y-6">{[["learning","Безперервне навчання"],["autoDrafts","Автоматичні чернетки"],["weeklyDigest","Щотижневий підсумок"],["privacyMode","Приватний режим"]].map(([key,label]) => { const k=key as keyof AltrProfile["preferences"];return <div key={key} className="flex items-center justify-between gap-4"><span className="text-sm">{label}</span><Toggle label={label} active={profile.preferences[k]} onClick={() => persist({preferences:{...profile.preferences,[k]:!profile.preferences[k]}})} /></div>})}</div></Panel><Panel className="p-6"><div className="flex flex-wrap gap-3"><button onClick={leave} className="glass-button inline-flex items-center gap-2 rounded-full px-5 py-3 text-sm"><LogOut className="h-4 w-4" />Вийти</button><button onClick={remove} className="danger-button inline-flex items-center gap-2 rounded-full px-5 py-3 text-sm"><Trash2 className="h-4 w-4" />Видалити акаунт</button></div></Panel></div>
            </div>
          </motion.div>}
        </div>
      </div>
