...
export async function POST(request: NextRequest) {
  ...
  if (eventName?.startsWith("subscription_")) {
    ...
    const existing = subscriptionId ? await admin.from("altr_subscriptions").select("id").eq("lemon_squeezy_subscription_id", subscriptionId).maybeSingle() : { data: null };
  ...
  if (eventName === "order_created") {
    ...
    const existing = orderId ? await admin.from("altr_invoices").select("id").eq("lemon_squeezy_order_id", orderId).maybeSingle() : { data: null };
    if (existing.data?.id) await admin.from("altr_invoices").update(row).eq("id", existing.data.id);
    else await admin.from("altr_invoices").insert(row);
