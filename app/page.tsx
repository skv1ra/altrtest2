...
export default function Home(){
 ...
 return <main id="top" className="relative min-h-screen overflow-hidden bg-[#05080c] text-white">
  ...
  <section className="relative z-10 mx-auto grid min-h-screen max-w-7xl items-center gap-12 px-5 pb-20 pt-32 lg:grid-cols-[1fr_.95fr]">
   <div><motion.h1 {...enter} className="hero-gradient-sweep text-balance text-5xl font-medium leading-[.96] tracking-[-.07em] md:text-7xl lg:text-[6.7rem]">{t.heroTitle}</motion.h1><p className="mt-8 max-w-2xl text-lg leading-8 text-white/64">{t.heroSubtitle}</p><div className="mt-10 flex flex-col gap-3 sm:flex-row"><Link href="/auth?mode=register" className="future-button rounded-full px-6 py-4 text-center text-sm">{t.cta}</Link><a href="#how" className="glass-button rounded-full px-6 py-4 text-center text-sm">{t.secondary}</a></div><div className="mt-12 grid gap-3 text-xs uppercase tracking-[.18em] text-white/48 sm:grid-cols-3">{[t.statA,t.statB,t.statC].map(x=><div key={x} className="micro-panel"><span className="status-dot-css"/>{x}</div>)}</div></div><HeroOrb/>
  </section>
