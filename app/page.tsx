...
function BlueButton({ children }: { children: React.ReactNode }) {
  return (
    <a href="#product" className="premium-button group inline-flex items-center justify-center rounded-full px-6 py-4 text-sm font-medium tracking-[-0.01em] text-white transition duration-500">
      <span>{children}</span>
      <span className="ml-3 h-1.5 w-1.5 rounded-full bg-blue-200 shadow-[0_0_14px_rgba(147,197,253,.95)] transition duration-500 group-hover:scale-150" />
    </a>
  );
}
  ...
  return (
    <main id="top" className="relative min-h-screen overflow-hidden bg-[#090909] text-white selection:bg-blue-200 selection:text-black">
      ...
      <div className="pointer-events-none fixed inset-0 z-0">
        <motion.div animate={{ x: [0, 70, 0], y: [0, -45, 0] }} transition={{ duration: 20, repeat: Infinity, ease: "easeInOut" }} className="absolute right-[8%] top-[18%] h-[26rem] w-[26rem] rounded-full bg-blue-500/[0.085] blur-[120px]" />
        <motion.div animate={{ x: [0, -48, 0], y: [0, 55, 0] }} transition={{ duration: 24, repeat: Infinity, ease: "easeInOut" }} className="absolute left-[4%] top-[40%] h-[22rem] w-[22rem] rounded-full bg-violet-500/[0.045] blur-[120px]" />
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_0%,rgba(255,255,255,.06),transparent_38%)]" />
      </div>
      ...
      <section className="relative z-10 mx-auto grid min-h-screen max-w-7xl items-center gap-12 px-5 pb-20 pt-28 lg:grid-cols-[1.02fr_.98fr] lg:pt-24">
        <div className="max-w-3xl">
          <motion.p initial={{ opacity: 0, y: 16, filter: "blur(8px)" }} animate={{ opacity: 1, y: 0, filter: "blur(0px)" }} transition={{ duration: 0.85, ease: [0.16, 1, 0.3, 1] }} className="mb-7 flex items-center gap-3 text-sm text-white/38">
            <span>Altr</span>
            <AiMark />
            <span>{lang === "EN" ? "personal intelligence layer" : "персональний intelligence layer"}</span>
          </motion.p>
          ...
          <motion.h1
            ...
            className="hero-gradient-sweep text-balance text-6xl font-medium leading-[0.94] tracking-[-0.075em] md:text-8xl lg:text-[7.4rem]"
          >
            {t.heroTitle}
          </motion.h1>

          <motion.p initial={{ opacity: 0, y: 18, filter: "blur(10px)" }} animate={{ opacity: 1, y: 0, filter: "blur(0px)" }} transition={{ duration: 0.9, delay: 0.16, ease: [0.16, 1, 0.3, 1] }} className="mt-8 max-w-xl text-lg leading-8 tracking-[-0.02em] text-white/52 md:text-xl">
            {t.heroSubtitle}
          </motion.p>

          <motion.div initial={{ opacity: 0, y: 18, filter: "blur(8px)" }} animate={{ opacity: 1, y: 0, filter: "blur(0px)" }} transition={{ duration: 0.9, delay: 0.28, ease: [0.16, 1, 0.3, 1] }} className="mt-10">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {t.cards.map(([title, subtitle, body], index) => (
            <Reveal key={title} delay={index * 0.08}>
              <motion.article whileHover={{ y: -8 }} className="premium-card group min-h-[310px] rounded-[2rem] p-7 md:p-8">
                <div className="mb-16 flex items-center justify-between">
                  <span className="text-sm text-white/38">0{index + 1}</span>
                  <span className="h-1.5 w-1.5 rounded-full bg-white/18 transition group-hover:bg-blue-200 group-hover:shadow-[0_0_18px_rgba(147,197,253,.95)]" />
                </div>
      ...
      <section className="relative z-10 overflow-hidden px-5 py-24 md:py-32">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_35%,rgba(55,112,255,.10),transparent_45%)]" />
        <div className="relative mx-auto grid max-w-7xl items-center gap-8 rounded-[2.6rem] border border-white/[0.07] bg-white/[0.025] p-6 shadow-[0_28px_120px_rgba(0,0,0,.52)] backdrop-blur-2xl md:p-12 lg:grid-cols-[.9fr_1.1fr]">
      ...
      <footer className="relative z-10 border-t border-white/[0.06] px-5 py-10">
        <div className="mx-auto flex max-w-7xl flex-col gap-8 md:flex-row md:items-center md:justify-between">
          <a href="#top" className="flex items-center gap-2 text-sm font-medium text-white">
            <span>Altr</span>
            <AiMark />
          </a>
          <div className="flex flex-wrap gap-x-7 gap-y-3 text-sm text-white/35">
            <a className="footer-link" href="#product">{t.footer.product}</a>
            <a className="footer-link" href="#how">{t.footer.how}</a>
            <a className="footer-link" href="#memory">{t.footer.memory}</a>
            <a className="footer-link" href="#vision">{t.footer.vision}</a>
            <a className="footer-link" href="https://x.com">X</a>
            <a className="footer-link" href="https://github.com">GitHub</a>
            <a className="footer-link" href="#">{t.footer.privacy}</a>
            <a className="footer-link" href="#">{t.footer.terms}</a>
          </div>
