...
export function HeroOrb({ quiet = false }: { quiet?: boolean }) {
  ...
  return <div className={`activation-visual relative mx-auto h-[470px] w-full max-w-[540px] overflow-hidden rounded-[32px] ${quiet?"activation-visual-quiet":""}`}>
    ...
    <div className="absolute left-6 top-6 flex items-center gap-2 rounded-full border border-cyan-200/[0.10] bg-black/20 px-3 py-1.5 text-[10px] uppercase tracking-[0.22em] text-cyan-100/60 backdrop-blur-md"><span aria-hidden="true" className="h-1.5 w-1.5 rounded-full bg-cyan-200" />learning stream</div><div className="absolute bottom-6 right-6 rounded-full border border-white/[0.08] bg-white/[0.035] px-3 py-1.5 text-[10px] uppercase tracking-[0.22em] text-white/48 backdrop-blur-md">tone / context / memory</div>
  </div>;
}
