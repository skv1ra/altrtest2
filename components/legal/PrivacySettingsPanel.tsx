...
import { getCurrentConsent, hasValidConsent, recordConsent, withdrawConsent } from "@/lib/legal/consent-store";
...
const copy = {
  EN: {
    ...
    title: "Control what Altr learns.",
    subtitle: "Review consent, stop future processing, remove local prototype data, or export what this browser stores.",
    conversation: "Conversation processing",
    conversationBody: "Allows Altr to process chats, email, and connected messages you choose to import in order to analyze tone, context, and response patterns.",
    memory: "Personal AI memory",
    memoryBody: "Allows Altr to create and update derived memory, communication patterns, preferences, context, routines, and inferred profile information.",
    granted: "Granted",
    notGranted: "Not granted",
    enable: "Enable",
    withdraw: "Withdraw",
    effectConversation: "Withdrawing stops future imports and disconnects local prototype sources.",
    effectMemory: "Withdrawing stops future AI-memory updates. Existing local memory remains until you delete it.",
    terms: "Terms version",
    privacy: "Privacy version",
    dataTools: "Data controls",
    deleteMemory: "Delete AI memory",
    deleteConversations: "Delete imported conversation data",
    export: "Export available local data",
    deleteAll: "Request complete deletion",
    cookies: "Cookie preferences",
    deletedMemory: "Local AI-memory demo data deleted.",
    deletedConversations: "Local conversation demo data deleted and sources disconnected.",
    prototype: "Prototype notice",
    prototypeBody: "Accounts, consent records, and dashboard data are stored in localStorage in this build. Production consent logging and deletion require a server-side database, verified email workflow, and provider deletion jobs.",
    date: "Recorded",
    policyLinks: "Legal documents",
  },
  UA: {
    eyebrow: "ПРИВАТНІСТЬ І ДАНІ",
    title: "Керуй тим, чого навчається Altr.",
    subtitle: "Переглядай згоди, зупиняй майбутню обробку, видаляй локальні прототипні дані або експортуй те, що зберігається в цьому браузері.",
    conversation: "Обробка переписок",
    conversationBody: "Дозволяє Altr обробляти чати, email і підключені повідомлення, які ти вирішуєш імпортувати, для аналізу тону, контексту та моделей відповідей.",
    memory: "Персональна AI-памʼять",
    memoryBody: "Дозволяє Altr створювати й оновлювати похідну памʼять, патерни спілкування, уподобання, контекст, звички та виведену профільну інформацію.",
    granted: "Надано",
    notGranted: "Не надано",
    enable: "Увімкнути",
    withdraw: "Відкликати",
    effectConversation: "Відкликання зупиняє майбутні імпорти й відключає локальні прототипні джерела.",
    effectMemory: "Відкликання зупиняє майбутні оновлення AI-памʼяті. Наявна локальна памʼять залишається, доки ти її не видалиш.",
    terms: "Версія Умов",
    privacy: "Версія Політики",
    dataTools: "Керування даними",
    deleteMemory: "Видалити AI-памʼять",
    deleteConversations: "Видалити імпортовані переписки",
    export: "Експортувати доступні локальні дані",
    deleteAll: "Запросити повне видалення",
    cookies: "Налаштування cookie",
    deletedMemory: "Локальні демонстраційні дані AI-памʼяті видалено.",
    deletedConversations: "Локальні демонстраційні дані переписок видалено, джерела відключено.",
    prototype: "Повідомлення про прототип",
    prototypeBody: "У цій збірці акаунти, записи згод і дані кабінету зберігаються в localStorage. Для production потрібні серверна база, перевірка через email і задачі видалення у провайдерів.",
    date: "Записано",
    policyLinks: "Юридичні документи",
  },
} as const;
...
export function PrivacySettingsPanel({ profile, onProfileChange }: { profile: AltrProfile; onProfileChange: (profile: AltrProfile) => void }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("altr-consent-change", update);
    return () => window.removeEventListener("altr-consent-change", update);
  }, []);

  const consentState = useMemo(() => {
    void revision;
    return {
      conversation: getCurrentConsent(profile.id, "conversation_processing"),
      memory: getCurrentConsent(profile.id, "ai_memory"),
      terms: getCurrentConsent(profile.id, "terms"),
      privacy: getCurrentConsent(profile.id, "privacy_acknowledgement"),
    };
  }, [profile.id, revision]);

  const applyProfile = async (update: Partial<AltrProfile>) => {
  ...
  const toggleConsent = (type: "conversation_processing" | "ai_memory") => {
    ...
    if (active) {
      ...
      if (type === "conversation_processing") {
        void applyProfile({ connections: { email: false, calendar: false, messages: false, workspace: false } });
      } else {
        void applyProfile({ preferences: { ...profile.preferences, learning: false } });
      }
    } else {
      recordConsent({ userId: profile.id, consentType: type, granted: true, source: "settings", locale: lang });
      if (type === "ai_memory") void applyProfile({ preferences: { ...profile.preferences, learning: true } });
    }
    setRevision((value) => value + 1);
  };

  const deleteMemory = () => {
    void applyProfile({ trainingProgress: 0, stats: { ...profile.stats, memories: 0 } });
    setMessage(t.deletedMemory);
  };

  const deleteConversations = () => {
    void applyProfile({ stats: { ...profile.stats, conversations: 0 }, connections: { email: false, calendar: false, messages: false, workspace: false } });
    setMessage(t.deletedConversations);
  };

  const exportData = () => {
    const payload = exportLocalAltrData();
    const blob = new Blob([JSON.stringify(payload, null, 2)], { type: "application/json" });
    const url = URL.createObjectURL(blob);
    const anchor = document.createElement("a");
    anchor.href = url;
    anchor.download = `altr-export-${new Date().toISOString().slice(0, 10)}.json`;
    anchor.click();
  ...
  return (
    <div>
      ...
      <h1 className="mt-4 text-4xl font-medium tracking-[-.055em] sm:text-5xl">{t.title}</h1>
      <p className="mt-3 max-w-2xl leading-7 text-white/38">{t.subtitle}</p>
