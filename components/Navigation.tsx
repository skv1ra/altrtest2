...
export function Navigation({ lang, setLang }: { lang: Lang; setLang: (lang: Lang) => void }) {
  ...
  useEffect(() => {
    let active = true;
    const sync = () => {
      void Promise.resolve(getCurrentProfile()).then((p) => {
        if (active) setSignedIn(Boolean(p));
      });
    };
    void sync();
  ...
  const links = [
    ...
    { href: "/assistants", label: t.assistants },
  ...
  return (
    ...
    >
      <nav
        aria-label="Primary"
        className="control-bar mx-auto flex h-16 max-w-7xl items-center justify-between px-4 md:px-6"
      >
        ...
        <div className="hidden items-center gap-7 md:flex">
          {links.map((x) => (
            <Link key={x.href} href={x.href} className="nav-link">
              {x.label}
            </Link>
          ))}
        </div>
        <div className="flex items-center gap-6">
          ...
          <Link
            ...
            className="profile-nav-button hidden sm:inline-flex"
          >
          ...
          <button
            ref={trigger}
            type="button"
            className="profile-nav-button md:hidden"
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -8 }}
            className="fixed inset-0 z-[-1] bg-black/70 px-4 pt-24 md:hidden"
            onMouseDown={(e) => {
          ...
          >
            <div
              ...
              aria-modal="true"
              aria-label={t.menu}
              className="mobile-menu-panel mx-auto max-w-md rounded-3xl p-4"
            >
              {links.map((x) => (
                <Link key={x.href} href={x.href} onClick={close} className="mobile-menu-link">
                  {x.label}
                </Link>
              ))}
            </div>
