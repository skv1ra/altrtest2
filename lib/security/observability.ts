...
export function safeErrorResponse(
  request: NextRequest,
  error: unknown,
  options: { code: string; status?: number; publicMessage?: string },
) {
  const requestId = getRequestId(request);
  structuredLog("error", "api.request_failed", {
    requestId,
  ...
  return NextResponse.json(
    { error: options.publicMessage ?? options.code, code: options.code, requestId },
    { status: options.status ?? 500, headers: { "x-request-id": requestId } },
  );
}

export function requestLogContext(request: NextRequest, extra: Record<string, unknown> = {}) {
  return {
