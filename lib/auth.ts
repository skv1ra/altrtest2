...
export async function getCurrentProfile(): Promise<AltrProfile | null> {
  try {
    const payload = await api<{ profile: AltrProfile | null }>("/api/me");
...
export async function registerAccount(input: { name: string; email: string; password: string; policyVersion: string }) {
  return api<{ profile: AltrProfile }>("/api/auth/register", {
    method: "POST",
...
export async function signInAccount(email: string, password: string) {
  return api<{ profile: AltrProfile }>("/api/auth/login", {
    method: "POST",
...
export async function updateCurrentProfile(update: Partial<AltrProfile>) {
  return api<{ profile: AltrProfile }>("/api/me", {
    method: "PATCH",
...
export async function deleteCurrentAccount(confirmText = "DELETE") {
  await api<{ ok: true }>("/api/me", {
    method: "DELETE",
    body: JSON.stringify({ confirmText }),
  });
}

export function redeemPromoCode(): never {
  throw new Error("PROMO_CODES_DISABLED_IN_PRODUCTION");
}

export function activatePaidSubscription(): never {
  throw new Error("PAID_PLANS_CAN_ONLY_BE_ACTIVATED_BY_VERIFIED_LEMON_SQUEEZY_WEBHOOKS");
}
