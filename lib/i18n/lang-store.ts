...
export function useLang(_initial:Lang=DEFAULT_LANGUAGE){const[lang,setLangState]=useState<Lang>(DEFAULT_LANGUAGE);useEffect(()=>{const stored=getStoredLanguage();setLangState(stored);document.documentElement.lang=stored==="UA"?"uk":"en";const sync=(event:Event)=>{const next=(event as CustomEvent<Lang>).detail??getStoredLanguage();setLangState(next);document.documentElement.lang=next==="UA"?"uk":"en"};addEventListener("altr-language-change",sync);return()=>removeEventListener("altr-language-change",sync)},[]);const setLang=(next:Lang)=>{setLangState(next);setStoredLanguage(next)};return[lang,setLang]as const}
