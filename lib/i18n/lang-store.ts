...
export function getStoredLanguage(): Lang {
  ...
  try {
    return localStorage.getItem(LANGUAGE_STORAGE_KEY) === "UA" ? "UA" : "EN";
  } catch {
...
export function useLang(_initial: Lang = DEFAULT_LANGUAGE) {
  ...
  useEffect(() => {
    ...
    addEventListener("altr-language-change", sync);
    return () => removeEventListener("altr-language-change", sync);
  }, []);
