...
if (appUrl) {
  try {
    ...
    if (["localhost", "127.0.0.1"].includes(url.hostname)) errors.push("NEXT_PUBLIC_APP_URL must not point to localhost");
...
for (const name of ["NEXT_PUBLIC_APP_URL", "NEXT_PUBLIC_SUPABASE_URL", "NEXT_PUBLIC_SUPABASE_ANON_KEY", "SUPABASE_SERVICE_ROLE_KEY", "LEMONSQUEEZY_API_KEY", "LEMONSQUEEZY_STORE_ID", "LEMONSQUEEZY_WEBHOOK_SECRET", "LEMONSQUEEZY_PERSONAL_VARIANT_ID", "LEMONSQUEEZY_WORK_VARIANT_ID", "OPENAI_API_KEY", "PRIVACY_EMAIL", "SUPPORT_EMAIL"]) {
  if (env[name] && placeholder(env[name])) errors.push(`${name} still contains a placeholder`);
}

const legalPath = path.join(root, "lib/legal/legal-config.ts");
...
const unsafeFlags = ["NEXT_PUBLIC_ENABLE_ANALYTICS", "NEXT_PUBLIC_ENABLE_MARKETING", "ALLOW_UNSAFE_EVAL", "DISABLE_AUTH", "SKIP_PRODUCTION_VERIFICATION"];
